...
function AnimatedCounter({ target, suffix = "" }: { target: number; suffix?: string }) {
  ...
  return (
    <div ref={ref} className="font-heading text-3xl md:text-4xl font-bold text-ice">
      {count}{suffix}
    </div>
  );
}
...
export function About() {
  return (
    <section id="about" className="relative bg-slate-dark py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center mb-16">
          <div className="relative">
            ...
            <div className="absolute -bottom-5 -right-3 md:-right-5 bg-forest rounded-xl p-5 shadow-2xl shadow-black/30">
              <p className="font-heading text-2xl font-bold text-white">15+</p>
              <p className="text-white/60 text-xs tracking-wider uppercase mt-0.5">Years of<br />Excellence</p>
            </div>
          </div>

          <div>
            <p className="text-forest-light font-heading font-medium tracking-[0.2em] uppercase text-xs mb-3">
              About Silverado
            </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((stat) => (
            <div key={stat.label} className="text-center glass rounded-xl p-6">
              {stat.isDecimal ? (
                <div className="font-heading text-3xl md:text-4xl font-bold text-ice">{stat.value}</div>
              ) : (
                <AnimatedCounter target={stat.value} suffix={stat.suffix} />
              )}
              <p className="text-white/35 text-xs tracking-wider uppercase mt-2">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
